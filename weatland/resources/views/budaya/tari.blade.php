...
@section('konten')
    <div class="container py-5">
        ...
        <div class="d-flex align-items-center justify-content-between mb-5">
            <h1>Artikel Tari Pulau Kalimantan</h1>
            ...
            @foreach ($items as $item)
                <div class="col-md-4">
                    <div class="card border-0 mb-3">
                        <img src="{{ url('storage/' . $item->image) }}" alt="" wdiht="100px" height="250px"
                            class="card-img-top">
                        <div class="card-body">
                            <h5 class="card-title">{{ $item->title }}</h5>
                            <p class="mb-0 text-secondary">{{ substr($item->description, 0, 30) }}.....</p>
                            <a href="{{ route('readmore.tari', ['id' => $item->id]) }}" class="btn btn-success">Read
                                More</a>

                            <form action="{{ route('tari.destroy', $item->id) }}" method="POST" style="display: inline">
                                @csrf
                                @method('DELETE')
