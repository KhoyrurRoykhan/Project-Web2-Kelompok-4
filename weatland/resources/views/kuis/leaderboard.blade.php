...
<html lang="en">
  ...
  <body>
    ...
    <tr>
        ...
        <td class="points">
            @if (($items->currentPage() - 1) * $items->perPage() + $index + 1 === 1)
                {{ $item->skor }} <img class="gold-medal" src="https://github.com/malunaridev/Challenges-iCodeThis/blob/master/4-leaderboard/assets/gold-medal.png?raw=true" alt="gold medal"/>
            @else
                {{ $item->skor }}
            @endif
        </td>
    ...
    <div id="buttons" style="margin-bottom: 20px">
        <form action="{{ route('menu.quis') }}">
            <button style="background-color: rgb(218, 75, 75)" class="continue">Keluar</button>
        </form>
    </div>
