    ...
    <main>
      ...
      <form style="font-family: 'Rubik', sans-serif;">
        <div class="quiz-options">
          <input type="radio" class="input-radio" id="one-a-{{ $key + 1 }}" name="yes-{{ $key + 1 }}" value="{{ $item->option_a }}" required>
          <label style="background-color: #a8a8a8" class="radio-label" for="one-a-{{ $key + 1 }}" onclick="checkAnswer('{{ $item->option_a }}', 'one-a-{{ $key + 1 }}','{{$item->Jawaban}}')">
            <span class="alphabet">A</span> {{ $item->option_a }} <img class="icon">
          </label>
          <input type="radio" class="input-radio" id="one-b-{{ $key + 1 }}" name="yes-{{ $key + 1 }}" value="{{ $item->option_b }}">
          <label style="background-color: #a8a8a8" class="radio-label" for="one-b-{{ $key + 1 }}" onclick="checkAnswer('{{ $item->option_b }}', 'one-b-{{ $key + 1 }}','{{$item->Jawaban}}')">
            <span class="alphabet">B</span> {{ $item->option_b }} <img class="icon">
          </label>
          <input type="radio" class="input-radio" id="one-c-{{ $key + 1 }}" name="yes-{{ $key + 1 }}" value="{{ $item->option_c }}">
          <label style="background-color: #a8a8a8" class="radio-label" for="one-c-{{ $key + 1 }}" onclick="checkAnswer('{{ $item->option_c }}', 'one-c-{{ $key + 1 }}','{{$item->Jawaban}}')">
            <span class="alphabet">C</span> {{ $item->option_c }} <img class="icon">
          </label>
        </div>
        @if ($key + 1 < count($items))
          <a style="background-color: #a8a8a8" id="btn" onclick="nextSection({{ $key + 1 }})">Next</a>
          @else
          <a style="background-color: #a8a8a8" id="btn" onclick="window.location.href='#game-over'">Next</a>
        @endif
      </form>
    </main>
  </section>
  ...
  <section id="game-over">
    <div class="game-over-content" >
      <div class="over-text-cont">
        ...
        <form style="margin-bottom: 10px" action="{{ route('store.leaderboard') }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="">
                <input type="text" class="form-control" name="nama" id="nama_user" style="visibility: hidden" value="">
            </div>
            <div class="">
                <input type="number" class="form-control" name="skor" id="skor_user" style="visibility: hidden" value="">
            </div>
        ...
        {{--  --}}
        <form action="{{ route('kuis.mulaikuis') }}" method="get"
