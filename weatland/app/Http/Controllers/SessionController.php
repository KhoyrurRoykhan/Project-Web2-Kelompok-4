...
class SessionController extends Controller
{
    ...
    function login(Request $request)
    {
        ...
        if (Auth::attempt($infologin, $remember)) {
            $cookie = cookie('remember_me', json_encode($infologin), 1440); // set cookie dengan waktu berlaku 1 hari (1440 menit)

            return redirect('home')->with("success", 'Berhasil Login')->cookie($cookie); // mengirimkan cookie ke browser
        } else {
            return redirect('sesi')->with('error', 'Username dan Password Salah');
        }
    }
